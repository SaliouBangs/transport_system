...
def build_user_permissions(user):
    ...
    return {
        ...
        "can_access_gps": user_has_role(user, ROLE_COMPTABLE, ROLE_LOGISTIQUE, ROLE_TRANSITAIRE, ROLE_DIRECTEUR),
        "can_access_prospects": user_has_role(user, ROLE_COMMERCIAL, ROLE_RESPONSABLE_COMMERCIAL, ROLE_DIRECTEUR),
        "can_add_prospects": user_has_role(user, ROLE_COMMERCIAL, ROLE_RESPONSABLE_COMMERCIAL, ROLE_DIRECTEUR),
        "can_delete_prospects": is_boss or user_has_role(user, ROLE_DIRECTEUR),
        "can_convert_prospects": is_boss or is_directeur_role,
        "can_access_clients": bool(getattr(user, "is_authenticated", False) and not is_caissiere),
        "can_add_clients": user_has_role(user, ROLE_COMMERCIAL, ROLE_RESPONSABLE_COMMERCIAL, ROLE_DIRECTEUR),
        "can_edit_clients": is_boss or user_has_role(user, ROLE_COMMERCIAL, ROLE_RESPONSABLE_COMMERCIAL, ROLE_DIRECTEUR),
        "can_delete_clients": is_boss or user_has_role(user, ROLE_DIRECTEUR),
        "can_manage_client_portfolios": is_boss or user_has_role(user, ROLE_RESPONSABLE_COMMERCIAL, ROLE_DIRECTEUR),
        ...
        "can_add_commandes": user_has_role(user, ROLE_COMMERCIAL, ROLE_RESPONSABLE_COMMERCIAL, ROLE_DIRECTEUR),
        "can_edit_commandes": is_boss or user_has_role(user, ROLE_COMMERCIAL, ROLE_RESPONSABLE_COMMERCIAL, ROLE_DIRECTEUR),
        ...
        "can_access_operations_secretaire": user_has_role(user, ROLE_SECRETAIRE, ROLE_DIRECTEUR),
        ...
        "can_access_operations_chef_chauffeur": user_has_role(user, ROLE_CHEF_CHAUFFEUR, ROLE_DIRECTEUR),
        ...
        "can_access_documents": user_has_role(user, ROLE_LOGISTIQUE, ROLE_MAINTENANCIER, ROLE_DGA, ROLE_DIRECTEUR),
        ...
        "can_access_rapport_global": bool(getattr(user, "is_authenticated", False) and not is_caissiere),
        "can_access_reports_center": bool(getattr(user, "is_authenticated", False) and not is_caissiere),
